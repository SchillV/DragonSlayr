...
struct OverlayQuad {
    glm::vec2 pos{0.0f};  // pixels, top-left origin (of the unrotated quad)
    glm::vec2 size{0.0f}; // pixels
    glm::vec4 uv_rect{0.0f, 0.0f, 1.0f, 1.0f};
    float layer = 0.0f;   // overlay texture array layer (0 = white)
    float rot = 0.0f;     // radians, rotation around the quad center
    glm::vec2 _pad{0.0f}; // keeps color vec4-aligned for the GPU layout
    glm::vec4 color{1.0f};
};
...
struct FrameView {
    ...
    std::vector<OverlayQuad> overlay_text; // same pipeline, sampled from the font atlas
};
