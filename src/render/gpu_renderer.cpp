...
bool GpuRenderer::init(SDL_Window* window) {
    ...
    if (!nearest_sampler_ || !create_world_pipeline()) {
        return false;
    }
    return true;
}
...
bool GpuRenderer::create_world_pipeline() {
    ...
    // TODO(M2): switch to BACK culling once the dungeon mesh builder emits
    // consistent winding; the M1 diorama is double-sided.
...
void GpuRenderer::set_world_textures(std::span<const Image> layers) {
    if (world_atlas_) {
        SDL_WaitForGPUIdle(dev_.handle());
...
void GpuRenderer::render(const FrameView& view) {
    ...
    if (world_pipeline_ && world_vbuf_ && world_ibuf_ && world_atlas_ && world_index_count_ > 0) {
        ...
        const glm::mat4 proj =
            glm::perspective(glm::radians(view.camera.fov_deg), aspect, 0.05f, 100.0f);

        CameraUbo cam{proj * view_mat};
        SDL_PushGPUVertexUniformData(cmd, 0, &cam, sizeof(cam));
        FogUbo fog{fog_color_density_};
